...
- Checkout order terhubung ke RajaOngkir untuk ongkir dan Xendit untuk pembayaran.
...
The current seed creates 20 grocery categories and 87 realistic products. Product records include stable SKU, brand, short info, storage instruction, weight in grams, active status, branch inventory, discounts, and local image paths generated by the web asset script.

After pulling the product merchandising migration, apply it to the target database before running the seed:
...
Response akan membawa data order, detail shipping, dan object payment dari `invoice_url` Xendit jika berhasil dibuat:
...
```json
{
  ...
  "payment": {
    "method": "xendit",
    "redirectUrl": "https://checkout.xendit.co/web/INVOICE_ID",
    "invoiceUrl": "https://checkout.xendit.co/web/INVOICE_ID",
